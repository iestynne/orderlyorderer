...
| B5 | **Beating 2-6 will invalidate the corpus.** | `royal_boon2` sits on 2-6 floor 75. Collecting it makes `level_scripts.lua` inject a Rapier — and move walls, including *raising* ten in 2-1 — in every tower where a Dark Crown is held. The 326/326 sweep is valid for the current account state; re-run it after 2-6, and expect the sim to need the account state as a `SimInput` before it agrees again. |
